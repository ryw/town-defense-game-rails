class Game < ActiveRecord::Base
  ...
  def h_attract
    ...
    candidates.uniq.each do |c|
      ...
      if chance > (c.earned_ep+20) or self.engaged_heros.count == 0
        ...
        c.estatus = 2
        c.save
        message += "#{c.name_with_title} has arrived to the battle and watches on. " 
        message += "<span class=\"rolldata\">#{chance} vs. #{c.rank*20}</span><br />" 
      end
    end 
    message
  end
  ...
  def h_add
    ...
    if candidate
      if chance > (candidate.earned_ep+20) or self.engaged_heros.count == 0
        ...
        candidate.save
        message += "#{candidate.name_with_title} has joined the fray! " 
        message += "<span class=\"rolldata\">#{chance} vs. #{candidate.rank*30}</span><br />" 
      end
    else
  ...
  def c_attract
    ...
    if chance > 40 or self.engaged_foes.count == 0
      ...
      message += "#{foe_name} the #{candidate.name} has arrived to the battle and watches on. "
